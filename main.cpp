...
#if defined(NDEBUG)
# error "Templecoin cannot be compiled without assertions."
#endif
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Templecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Templecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Templecoin: 2.5 minutes
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Trump promises to reduce federal regulations to pre-1960 level USA TODAY";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Templecoin: increase each by adding 2 to bitcoin's value.
